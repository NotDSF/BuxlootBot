...
module.exports = {
    async execute(interaction: ChatInputCommandInteraction<CacheType>) {
        ...
        const result = await BuxLoot.RemoveBalance(username, amount);
        ...
        Embed.setDescription(`We recieved your request to remove **${amount} R$** from this users balance\nThis users balance is now: **${stats.body.Data.balance} R$**`);
    ...
    data: new SlashCommandBuilder()
        ...
        .setDescription("Remove balance from a site user")
        .addStringOption((user) => user.setName("username").setRequired(true).setDescription("The site users roblox username"))
        .addNumberOption((amount) => amount.setName("amount").setRequired(true).setDescription("The amount you'd like to remove").setMinValue(1).setMaxValue(999999))
